...
- `esphome/ble-scan-example.yaml` — scan-only config for finding the real BMS BLE endpoint
- `esphome/ks-bms-example.yaml` — working example using `syssi/esphome-ks-bms`
- `home-assistant/template-sensors.yaml` — example HA template ideas for usable battery metrics

## Hardware used

- Docan Panda 48kWh 51.2V battery pack with KS-style BMS
- ESP32 running ESPHome
- Home Assistant

## Important notes

### 1) Some battery systems expose a Bluetooth radio for the display/tablet/touchscreen layer, while the BMS telemetry comes from a **different BLE module**.
...
### 2) Keep BLE enabled after reboot
Using:
